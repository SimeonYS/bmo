...
class BmoPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bmo.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bmo`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bmo WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bmo (date, title, link, content)"
